...
class XYStageDriver:
    ...
    def image_frame(self):
        ...
        tk.Button(btn_frame, text="Create Path", command=self.generate_path_from_image, height=1).grid(row=0, column=2,
                                                                                                       padx=2)
        tk.Button(btn_frame, text="Clear", command=self.clear_data, height=1).grid(row=0, column=3, padx=2)

        # Scaling in one row
        scale_frame = ttk.Frame(image_frame)
        scale_frame.grid(row=1, column=0, columnspan=2, padx=pad, pady=pad)
        ...
        self.abortbtn.grid(row=2, column=0, columnspan=2, padx=pad, pady=pad)

        # Canvas - smaller
        self.imagecanvas = tk.Canvas(image_frame, height=200, width=300, bg='lightgray')
        self.imagecanvas.grid(row=3, column=0, columnspan=2, padx=pad, pady=pad)

        # Size box controls - compact
        size_frame = ttk.LabelFrame(image_frame, text="Size Box")
        size_frame.grid(row=4, column=0, columnspan=2, padx=pad, pady=pad, sticky='ew')
...
###
    ...
    #     """Generate scaled path data from stored contours"""
    #     if not hasattr(self, 'raw_contours'):
    #         print("No contours loaded!")
    #         return
    #
    #     # Clear existing data
    #     self.datax = []
    ...
    #
    #     # Update laser power from GUI if not already set
    #     if self.laserpower == 0:
    #         self.set_laser_power()
    #         if self.laserpower == 0:  # Still 0 after trying to set
    #             self.laserpower = 100  # Default
    #
    ...
    #             self.laserstate.append(0)  # Laser OFF for positioning
    #
    #             # Draw the contour with laser ON
    ...
    #                 self.laserstate.append(self.laserpower)  # Laser ON with current power
    #
